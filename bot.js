...
client.on('message', message => {
    if (message.author.bot) return;
     if (message.content === prefix + "help") {
		 message.channel.send('**تم راسال في خاص**');
...
•========================================• 
                     •اوامر ادارية•
•========================================• 
 🔱 %ban @name time => حتة تبند شخص

 🔱 %clear => حتة تمسح رساءل روم 
 
 🔱 %mute @name time => حتة تعطي حد ميوت

 🔱 %unmute @name => حتة توخر ميوت من شخص

 🔱 %kick @name => حتة تطر حد من سيرفر بدون تبنيد

 🔱 %bc {masseg} => حتة ترسل رسالة لجميع الي بل سيرفر في خاص 

•========================================• 
                       •اوامر عامة•
•========================================•
🔱 %mcskin {nameskin} => يضهر لك سكن حق ماين كرافت
🔱 %bot => يضهر لك معلومات بوت
🔱 %server => يضهر لك معلومات سيرفر
🔱 %ping => حتة تعرف كم بنق بوت
🔱 %id => حتة تعرف معلومات عن حسابك
🔱 %avatar => حتة تضهر لك صورا خاصة بحسابك ديسكورد
🔱 %say {masseg} => يعيد كلام التي كتبته وبشكل احلة و اجمل 
═══════════════════════════════════
:heart: شكرا لك لستعمال بوت X-Bot :heart

--(Bot By @50%.#0063)-- :heart:🔱
...
client.on('guildMemberAdd', member => {
    ...
    client.channels.get('511195701615525909').setName(`⟫『 ${memberCount} member count 』⟪`);
    client.channels.get('510889296811786281').setName(`⟫『 ${botCount} عدد البوتات 』⟪`);
});
...
client.on('roleUpdate', (oldRole, newRole) => {
    ...
    oldRole.guild.fetchAuditLogs().then(logs => {
        ...
        if(oldRole.name !== newRole.name) {l
...
client.login('token');, oldRole.guild.iconURL
...
client.on('guildUpdate', (oldGuild, newGuild) => {l
    ...
    oldGuild.fetchAuditLogs().then(logs => {
        ...
        if(oldGuild.verificationLevel !== newGuild.verificationLevel) {
            ...
            if(oldGuild.verificationLevel === 1) {
                var oldVerLvl = 'Easy';const Discord = require('discord.js');
const client = new Discord.Client();
const prefix = ("%")
client.on('ready', () => {
  console.log(`Logged in as ${client.user.tag}!`);
...
client.on('message', message => {
    if (message.author.id === client.user.id) return;
        ...
        message.channel.sendMessage('***بوت بانق***  `' + `${client.ping}` + ' ms`');
    }
});
...
var prefix = "%"//البرفكس
   ...
   if(message.content.startsWith(prefix + "ban")) {
       ...
       if(toBan.id === ("حط ايديك")) return message.reply("**انا لا استطيع طرد نفسي**");
...
message.channel.send(`صورا حقتك ${user} رابط : ${user.avatarURL}`);
}
});
